...
def _pick_voice() -> str:
    """Alege vocea folosita pentru acest video (aceeasi voce pentru tot textul)."""
    tts_cfg = CONFIG["tts"]
...
def generate_voice_over_segments(segments: list[dict], output_dir: Path) -> tuple[Path, list[dict]]:
    ...
    for i, seg in enumerate(segments):
        ...
        word_boundaries = asyncio.run(_synthesize_with_timings(raw_text, voice, rate, pitch, seg_path))
